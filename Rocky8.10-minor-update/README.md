...
인터넷 연결이 불가능한 환경에서 스크립트와 같은 디렉터리에 있는 `Rocky-8.10-x86_64-dvd1.iso` 파일을 local repository로 사용하고, Rocky Linux 8.10의 BaseOS/AppStream repository 기준으로 `dnf distro-sync`를 수행합니다.
...
- ISO 위치: 스크립트와 같은 디렉터리의 `Rocky-8.10-x86_64-dvd1.iso`
- ISO SHA256: `642ada8a49dbeca8cca6543b31196019ee3d649a0163b5db0e646c7409364eeb`
- Rocky Linux GPG key: `/etc/pki/rpm-gpg/RPM-GPG-KEY-rockyofficial`
- `/mnt` 디렉터리 존재
- 스크립트 실행 현재 경로가 `/root` 또는 `/home` 하위여야 함
- 필요 명령어: `awk`, `basename`, `cat`, `cp`, `date`, `df`, `dirname`, `dnf`, `grep`, `id`, `ls`, `mkdir`, `mount`, `mountpoint`, `mv`, `pwd`, `rm`, `rpm`, `sed`, `sha256sum`, `sort`, `tail`, `umount`, `uname`
...
현재 작업 디렉터리가 `/root` 또는 `/home` 하위이면 하위 디렉터리 어디에서 실행해도 됩니다.
...
메뉴에서 선택한 작업이 `[FAIL]`로 중단되어도 스크립트 자체는 종료하지 않고 현재 메뉴로 돌아갑니다. ISO mount management 하위 메뉴에서 발생한 실패는 하위 메뉴로 돌아가고, 메인 메뉴에서 발생한 실패는 메인 메뉴로 돌아갑니다. ANSI 색상을 지원하는 터미널에서는 `[FAIL]`의 `FAIL` 글자만 빨간색으로 출력됩니다. `Run minor update` 실행 중 실패, 사용자 중단, HUP/TERM/INT 신호가 발생한 경우에는 종료 전에 repo 복원과 ISO 언마운트를 먼저 시도한 뒤 메뉴로 돌아갑니다.

## Hash 검증

외부 `.sha256` 파일은 사용하지 않습니다. 스크립트 내부의 `SCRIPT_SHA256` 값과 실행 중인 스크립트의 hash 값을 비교합니다.

ISO 파일도 존재 여부만 확인하지 않고 SHA256 hash를 함께 검증합니다. 스크립트와 같은 디렉터리에 `Rocky-8.10-x86_64-dvd1.iso`가 없거나, 파일이 있어도 hash가 아래 값과 다르면 작업을 진행하지 않습니다.

```text
642ada8a49dbeca8cca6543b31196019ee3d649a0163b5db0e646c7409364eeb
```

같은 디렉터리에 `Rocky-8.6-x86_64-dvd1.iso`처럼 다른 Rocky DVD ISO가 있으면 후보 파일로 안내하지만, update에는 사용하지 않습니다. 반드시 위 파일명과 hash가 일치하는 Rocky Linux 8.10 DVD ISO가 필요합니다.
...
- 스크립트 내부 hash 검증
- 스크립트 버전
- 스크립트 실행 경로가 `/root` 또는 `/home` 하위인지 여부
- Rocky Linux 8.x 여부
- 현재 OS 버전
- 현재 `rocky-release` 패키지 버전
- ISO 파일 존재 여부
- ISO 파일 SHA256 hash 일치 여부
...
`Mount ISO`는 스크립트와 같은 디렉터리의 `Rocky-8.10-x86_64-dvd1.iso` 파일 hash를 검증한 뒤 `/mnt/rocky810_iso`에 read-only loop mount하고, `BaseOS`와 `AppStream` 디렉터리가 존재하는지 확인합니다.

이미 `/mnt/rocky810_iso`가 마운트되어 있으면 해당 mount source가 스크립트와 같은 디렉터리의 `Rocky-8.10-x86_64-dvd1.iso`인지 확인합니다. 다른 source가 마운트되어 있으면 작업을 중단합니다.
...
진행 확인 프롬프트는 `[Y/N]`으로 표시됩니다. 대문자 `Y`를 입력한 경우에만 진행하며, `N`을 입력하면 사용자 취소로 중단합니다. 그 외 입력값은 잘못된 입력으로 안내한 뒤 다시 입력을 요구합니다.

### 4. Show update status

현재 update 상태를 확인합니다. 상태 파일이 있으면 주요 필드를 읽기 쉬운 형식으로 표시합니다.
...
| 경로 | 용도 |
|---|---|
| `/root/Rocky8.10_minor_update.sh` | 실행 스크립트 예시 |
| `<script directory>/Rocky-8.10-x86_64-dvd1.iso` | Rocky Linux 8.10 DVD ISO |
...
버전은 저장소 루트 `README.md`의 `Version Policy`에 정의된 공통 버전 관리 기준을 따릅니다.

이 스크립트를 수정하거나 새 버전을 기록하기 전에는 먼저 루트 `README.md`의 `Version Policy`를 확인해 `MAJOR`, `MINOR`, `PATCH` 중 어느 변경인지 판단합니다. 기존 Version History 항목에는 나중 변경을 덧붙이지 않고, 새 변경은 새 버전 항목으로 남깁니다.

버전을 올릴 때는 다음 항목을 함께 갱신합니다.

- 루트 `README.md`의 `Version Policy` 기준 확인
...
`Run minor update`가 실패, 사용자 중단으로 종료되면 스크립트는 다음 복원을 시도합니다.
...
- ISO 파일은 스크립트와 같은 디렉터리에 있어야 하며, 존재 여부와 SHA256 hash를 함께 검증합니다.
- 필요한 8.10 ISO가 없고 다른 Rocky DVD ISO만 있으면 후보 목록과 필요한 파일명/hash를 안내합니다.
- 메뉴에서 실행한 작업이 실패해도 전체 스크립트는 종료하지 않고 별도 복귀 경고 없이 현재 메뉴로 돌아갑니다.
- ANSI 색상을 지원하는 터미널에서는 `[FAIL]`의 `FAIL` 글자만 빨간색으로 강조합니다.
...
- 스크립트 실행 현재 경로를 검사해 `/root` 또는 `/home` 하위에서만 실행되도록 제한합니다.
...
- 스크립트에는 `curl`, `wget`, `http://`, `https://`, `mirrorlist`, `metalink`를 사용하는 외부 접속 로직이 없습니다.
- update 중 기존 `.repo` 파일은 임시 백업 위치로 이동되어 비활성화되고, DNF 실행 시 `--disablerepo='*'`와 ISO 기반 local repo만 `--enablerepo`로 지정합니다.
- 생성되는 임시 repository의 `baseurl`과 `gpgkey`는 모두 `file://` 경로를 사용합니다.
...
|---|---|---|
| 3.3.2 | 2026-04-21 | 진행 확인 프롬프트를 `[Y/N]`으로 변경하고 `Y`/`N`만 허용, README에 오프라인 repository 동작 설명 추가 |
| 3.3.1 | 2026-04-21 | 메뉴 상단 배너 추가, FAIL 발생 시 현재 메뉴로 복귀하도록 변경, ANSI 터미널에서 FAIL 글자만 빨간색 강조, 환경 점검의 중복 버전 출력 제거, 다른 Rocky DVD ISO 후보 안내 추가 |
| 3.3.0 | 2026-04-21 | ISO 위치를 스크립트와 같은 디렉터리로 변경하고, ISO 파일 존재 여부 확인 시 SHA256 hash 검증 추가 |
| 3.2.0 | 2026-04-21 | 실행 현재 경로를 `/root` 또는 `/home` 하위로 제한하고, status 메뉴 출력을 주요 필드 중심으로 정리 |
| 3.1.0 | 2026-04-21 | update 성공 후 ISO local repo 유지 선택 제거, 성공 시 항상 repo 복원과 ISO unmount 수행하도록 단순화 |
| 3.0.0 | 2026-04-21 | update 성공 후 기존 repo 복원 또는 현재 부팅 세션 동안 ISO local repo 유지 선택 추가, 새 완료 상태값과 완료 후 검증 조건 반영 |
