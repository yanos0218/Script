...
메뉴에서 선택한 작업이 `[FAIL]`로 중단되어도 스크립트 자체는 종료하지 않고 메인 메뉴로 돌아갑니다. `Run minor update` 실행 중 실패, 사용자 중단, HUP/TERM/INT 신호가 발생한 경우에는 종료 전에 repo 복원과 ISO 언마운트를 먼저 시도한 뒤 메뉴로 돌아갑니다.
...
- 메뉴에서 실행한 작업이 실패해도 전체 스크립트는 종료하지 않고 메뉴로 돌아갑니다.
...
|---|---|---|
| 3.3.0 | 2026-04-21 | ISO 위치를 스크립트와 같은 디렉터리로 변경하고, ISO 파일 존재 여부 확인 시 SHA256 hash 검증 추가, 메뉴 상단 배너 추가, FAIL 발생 시 메뉴로 복귀하도록 변경 |
